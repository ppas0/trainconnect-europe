...
router.post('/checkout/session', authenticate, async (req, res) => {
  try {
    ...
    // Create tickets for all items in cart immediately (demo mode)
...
router.get('/tickets/:id/pkpass', (req, res) => {
  ...
  res.send(Buffer.from(`DEMO_PKPASS:${ticket.pnr}:${ticket.from}→${ticket.to}`));
});
...
router.post('/push/test', authenticate, (req, res) => {
  res.json({ success: true, message: 'Push-Test (Demo – keine echte Nachricht gesendet)' });
});

router.post('/push/notify-delays', authenticate, (req, res) => {
  res.json({ success: true, notified: 0, message: 'Demo-Modus – keine Verspätungen aktiv' });
});
